  ...
  class Handler
    class << self
      ...
      def use_selenium
        ...
        Proc.new{|statement| puts "#{statement}:"; puts `#{statement}`}.call("google-chrome --version")
        Proc.new{|statement| puts "#{statement}:"; puts `#{statement}`}.call("/var/task/chromedriver --version")
        p "google-chromeコマンド実行 開始"
        p `google-chrome --no-sandbox --headless --window-size=1440,990 --disable-dev-shm-usage --disable-extensions start-maximized disable-infobars`
      ...
      def gen_accessor
        ...
        options = ::Selenium::WebDriver::Chrome::Options.new(
          # binary: "#{LAMBDA_TASK_ROOT}/headless-chromium"
        )
        options.headless!
